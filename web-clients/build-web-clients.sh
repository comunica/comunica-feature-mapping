#!/bin/bash
# Build web clients from all default configs of query-sparql-mapping
...
pushd engines/query-sparql-mapping >/dev/null
for config in ../config-query-sparql-mapping/config/*.json; do
  ...
  cat $cwd/web-clients/settings.json | \
    sed "s/__SUBTITLE__/Using $id config/; s~__SUBTITLE_HREF__~https://github.com/comunica/comunica-feature-mapping/blob/$GITHUB_SHA/engines/config-query-sparql-mapping/config/config-$id.json~" \
